...
from api.login.token import token
from api.prod.prod import Products
...
def get_products():
    """Get all products"""
    return jsonify(prod.getProducts()), 200
...
def create_product():
    ...
    try:
        prod.createProduct(name, price, title, image)
...
def get_product_by_id(product_id):
    row = prod.findProductById(product_id)
...
def update_product(product_id):
    ...
    prod.updateProduct(product_id, name, price, title)
...
def delete_product(product_id):
    row = prod.deleteProduct(product_id)
